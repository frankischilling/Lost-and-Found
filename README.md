...
- `GET /posts.php?id={uuid}` - Get single post (includes `follower_ids` array)
- `POST /posts.php` - Create new post (requires auth)
- `POST /posts.php?id={uuid}&action=follow` - Follow a post (requires auth)
- `POST /posts.php?id={uuid}&action=unfollow` - Unfollow a post (requires auth)
...
```javascript
// Follow a post
const followResponse = await fetch('https://project.frankhagan.online/posts.php?id=POST_UUID&action=follow', {
  method: 'POST',
  credentials: 'include'
});

const followData = await followResponse.json();
// Response: { status: 'success', message: 'Successfully followed post', is_following: true, follower_count: 1 }

// Unfollow a post
const unfollowResponse = await fetch('https://project.frankhagan.online/posts.php?id=POST_UUID&action=unfollow', {
  method: 'POST',
  credentials: 'include'
});

const unfollowData = await unfollowResponse.json();
// Response: { status: 'success', message: 'Successfully unfollowed post', is_following: false, follower_count: 0 }

// Check if you're following a post
const postResponse = await fetch('https://project.frankhagan.online/posts.php?id=POST_UUID', {
  credentials: 'include'
});
const post = await postResponse.json();

// Check if current user is in follower_ids array
const sessionResponse = await fetch('https://project.frankhagan.online/auth/session.php', {
  credentials: 'include'
});
const session = await sessionResponse.json();

const isFollowing = post.follower_ids && post.follower_ids.includes(session.user.id);
console.log('Is following:', isFollowing);
console.log('Follower count:', post.follower_ids ? post.follower_ids.length : 0);
```
...
- Users can follow posts to track if they match their lost/found items
- The `follower_ids` field is automatically managed when users follow/unfollow posts
- When a post has no followers, `follower_ids` is returned as an empty array `[]`
- All authenticated users can follow any post
